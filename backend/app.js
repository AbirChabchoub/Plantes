...
app.get("/ads/:id", (req, res) => {
    ...
    Ad.findOne({ _id: req.params.id }).then((findedObj) => { //_id !!! 5ater fel bd yetkteb hakek id sinn undefined
        if (findedObj) {
            res.status(200).json({
                ad: findedObj
            });
...
app.post("/adCategory", (req, res) => {
    ...
    const loginEmail = req.body.loginEmail;
    const loginPwd = req.body.loginPwd;
    User.findOne({ loginEmail, loginPwd }).then(
        (findedUser) => {
            if (!findedUser) {
                res.status(200).json({
                    message: '0'
...
app.get("/users/:id", (req, res) => {
    ...
    User.findOne({ _id: req.params.id }).then((findedObj) => { //_id !!! 5ater fel bd yetkteb hakek id sinn undefined
        if (findedObj) {
            res.status(200).json({
